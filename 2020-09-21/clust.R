...
for (key in c("PGs","17-ABC1Ks")) {
    for (typ in c("Scaled","Zscore")) {
        ...
        cat("'",paste0(pv$hclust$labels[pv$hclust$order],collapse="','"),"'",sep="")
...
for (key in c("PGs","17-ABC1Ks")) {
    for (typ in c("Scaled","Zscore")) {
        ...
        plot(pv, print.pv="au", print.num=FALSE, hang = -1, col.pv=c(au="blue"), cex.pv=1, cex = 1, frame.plot=FALSE, main=paste0("Heirarchical Clustering of Tissues: ",key," (using ",typ," protein abundance)"), sub="", xlab="", ylab="correlation-based distance")
        pvrect(pv, alpha=0.95, pv="au") # mark red boxes around the significant clusters
        if (save_plots) {
            dev.off()
        }
        ...
        # # print(D_dd)
        # D_hc = hclust(D_dd, method="average")
        # cat("'",paste0(D_hc$labels[D_hc$order],collapse="','"),"'",sep="")
        # png(filename = dendro_file, width=960, height=480, units="px")
