...
for key in A.keys():
    ...
    # note:
    # - to get protein_order that matches dendrogram: run pvclust() as in clust.R section "cluster the proteins (show p-values)" and then cat("'",paste0(pv$hclust$labels[pv$hclust$order],collapse="','"),"'",sep="")
    # - to get tissue_order that matches dendrogram: run clust.R "cluster the tissues (no p-values)" and copy-paste the tissue names (from "Zscore" version) in order
    if key == 'PGs':
    ...
    fig,axs = plt.subplots(figsize=(12,12))
    sns.heatmap(df.loc[protein_order,tissue_order], cmap="icefire", annot=False, ax=axs)
    axs.set_title(key)
    if show_plots:
        plt.show()
    else:
